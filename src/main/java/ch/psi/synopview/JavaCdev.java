...
//	int idebug = 0; // default for the synoptic viewer
	 ...
	 **
	 ** parameter: ControlSystem = cs... sHost = host to connect to the Gateway
	 * server port = port number of cdevGateway
	 **
	 */
	 ...
	 **
	 ** in the synoptic viewer, this method is called when the user clicks on a
	 * device in the drawing or select it from the tree. the result are
	 * displayed in the ControlPanel. ControlSystem provides the interface to
	 * this class
	 */
	 ...
	 ** getProperties
	 **
	 ** parameter: sDevice like "ALIVA-VG-5" return: Sting[] properties of the
	 * directory Service return something like "STATUS" or "I-READ"
	 */
	 ...
	 **
	 ** parameter: Strinng[] saProperties device Properties return: Sting[]
	 * saInterrestProperty Interrest Properties return something like "STATUS"
	 * or "I-READ"
	 */
	public String[] getInterrestProperty(String[] saProperties) {
		try {
			...
			// if the interrest group is defined, use only the properties define
			// in this group
			if (idSelected != null) {
				iLen = saProperties.length;
				sIGroup = idSelected.getName(); // the current selected
												// Interrest Group
				if (idSelected.getVName().size() == 0) {
					// in the InterrestGroup defined in the file
					// SV_interrest.txt there are no properties
					// defined for this group,
					// so all properties will be shown...
					return saProperties;
				}

				// durchsuche alle Properties und speichere die Anzahl der
				// ViewProperties...
				for (int i = 0; i < iLen; i++) {
					// teste ob die Interrest Gruppe dieses Property enth�lt
					if (idSelected.getVName().contains(saProperties[i])) {
						// System.out.println(sIGroup + " contains " +
						// saProperties[i]);
						iViewLen++;
					}
				}
	 ...
	 ** directoryService
	 **
	 ** parameter: sCommand tested with "query" and "queryAttributes" sDevice
	 * like "ALIVA-VG-5" return: Sting[] properties of the directoryService
	...
	public String[] directoryService(String sCommand, String sDevice) {
		try {
			...
			if (cDevice.send(sCommand, cData, cResult) != null) {
				return null; // error execute directoryService() (or device not
								// found...)
			}
	 ...
	 **
	 ** parameter: sDevice properties = the string array of properties to send
	 */
	...
	public String[] removeDevice(String[] saDP) {
		...
		int iLen;
		int iPointPos = 0; // the position of the ":" Char in the Device Name
							// (only old style)
		String saProperty[];
	 ...
	 *
	 * return: true = connected to the cdevGatewayServer false = not conneted
	 *
	...
	/**
	 * read the Interrest file that is used to store the information what
	 * properties will be displayed for each device
	 *
	 */
	public boolean readInterrestFile(String fileName) {
		...
		// after reading the file, add the elements to the InterrestGroup Choice
		// Widget
		try {
			InterrestsData id;
	 ...
	 **
	 ** parameter: properties = the string array of properties to send example:
	 * ALIVA-VG-5:STATUS ...
	 */
