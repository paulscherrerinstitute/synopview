package ch.psi.synopview.svp.visual.tree;
...
public class ScrPane extends Panel {
	...
	public boolean makeVisible(Rectangle item_rect) {
		...
		if (our_rect.y < item_rect.y && our_boty < item_boty) {
			// bottom over hang
		...
		} else if (our_rect.y > item_rect.y) {
			// top overhang
	...
	public void setSize(int width, int height) {
		if (width == 0 || height == 0)
			return; // catch intailisation problems
		...
		if (width != m_image_width || height != m_image_height) {
			...
			if (m_image != null)
				m_image.flush(); // clear any proccessing thats still to be done
		...
		if (hbar_size.height != 0)
			canvas_size.height -= hbar_size.height;
		else
		...
		if (m_vbar.isVisible()) {
			...
			if (vbar_size.width != 0)
				canvas_size.width -= vbar_size.width;
			else
				canvas_size.width -= 15;
		}
